...
const Hero = {
  async findActive() {
    const [rows] = await pool.query('SELECT * FROM hero WHERE isActive = 1 LIMIT 1');
  ...
  async create({ title, subtitle, buttonText, buttonLink, mediaType, mediaId = null, isActive = true }) {
    ...
    const [rows] = await pool.query('SELECT * FROM hero WHERE id = ?', [result.insertId]);
  ...
  async update(id, { title, subtitle, buttonText, buttonLink, mediaType, mediaId }) {
    ...
    const [rows] = await pool.query('SELECT * FROM hero WHERE id = ?', [id]);
    return withMedia(rows[0]);
  },
};
